...
gamma.est(cats$Hwt)
```
It mataches with the results in Lab3 #4
...
```{r}
rand.data<-rgamma(n=10000,shape=19,scale=0.56)
rand.data.mean<-mean(rand.data)
rand.data.var<-var(rand.data)
paste(c('The sample mean is',rand.data.mean))
paste(c('The sample variance is',rand.data.var))
```

```{r}
t_mean<-19*0.56
t_var<-19*0.56*0.56
paste(c('The theoretical mean is',t_mean))
paste(c('The theoretical variance is',t_var))
```
4. Plot the histogram of the random values, and add the curve of the theoretical probability density function.
```{r}
hist(rand.data,probability=TRUE,
     main='Histogram of 10000 random values from the gamma distribution')
curve(dgamma(x,shape=19,scale=0.56),add=TRUE,col='blue')
```
...
```{r}
gamma.est(rand.data)
est.shape<-gamma.est(rand.data)[[1]]
est.scale<-gamma.est(rand.data)[[2]]
paste(c('The difference between the estimated shape and true value is',est.shape-19))
paste(c('The difference between the estimated scale and true value is',est.scale-0.56))
```
...
```{r}
#Input of shape=a,scale=s,sample=n,number of repetitions=B
...
shapes <- rep(c(2,3), 5)
scales <- rep(c(8,13), 5)
rbind(shapes[1:7], scales[1:7]) # There was an error here originally
```
...
gamma.est.sim<-function(a,s,n,B){
  ...
  finalarr<-rbind(shapes[1:B],scales[1:B])
  return(finalarr)
}
...
```
The standard errors caluated by gamma.est.se matches with the standard deviation that calculated in Q8.
...
The answers are not exactly 1.6 and 0.54 because the rgamma function generated n random number.
