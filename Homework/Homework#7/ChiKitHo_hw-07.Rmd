...
```{r}
fish<-read.delim('http://www.faculty.ucr.edu/~jflegal/fish.txt',head=TRUE,sep=" ")
bevholt<-function(S=fish$S,beta1,beta2){
  1/(beta1+beta2/S)
}
x<-fish$S
plot(fish$S,fish$R,main='Scatterplot for Salmon Popultion',xlab='Spawners',ylab='Recruits')
curve(bevholt(x,0.001,0.85),add = TRUE,col='blue')
curve(bevholt(x,0.002,0.65),add = TRUE,col='red')
curve(bevholt(x,0.003,0.5),add = TRUE,col='darkgreen')
legend('bottomright',legend=c('beta1=0.0001, beta2=0.85',
                              'beta1=0.002, beta2=0.65',
                              'beta2=0.003,beta2=0.5'),
...
summary(fit)
plot(fish$S,fish$R,main='Scatterplot for Salmon Popultion',xlab='Spawners',ylab='Recruits')
abline(lm(r.inv~s.inv))
```
3. Find an estimate for the stable population level, where $R=S$ in the Beverton-Holt model.
```{r}
beta1_hat<-fit$coefficients[[1]]
beta2_hat<-fit$coefficients[[2]]
N<-(1-beta2_hat)/beta1_hat
N
```
...
rsq<-function(formula,data,indices){
  d<-data[indices,]
  d.inv<-1/d
  fit2<-lm(formula,data=d.inv)
  N<-(1-fit2$coefficients[[2]])/(fit2$coefficients[[1]])
  return(N)
}
results<-boot(data=fish,statistic = rsq, R=1000,formula=R~S)
results
...
```{r}
buffalo<-read.delim('http://www.faculty.ucr.edu/~jflegal/buffalo.txt')
Gaus<-density(buffalo$X126.4,kernel="gaussian")
Epan<-density(buffalo$X126.4,kernel='epanechnikov')
plot(Gaus,type='l',main='Annual snowfall in Buffalo, NY',
     xlab='Year',ylab='annual snowfall')
lines(Epan,col='blue')
legend('topright',legend=c('Gaussian','Epanechnikov'),col=c('black','blue'),lty=1)
```
6. Compare the estimates for different choices of bandwidth.
The default bandwith is 9.131, we will do the estimates with h2=5 h3=7 and h4=15
for both Gaussian and Epanechnikov kernels
```{r}
#Gaussian kernels
Gaus<-density(buffalo$X126.4,kernel="gaussian")
Gaus2<-density(buffalo$X126.4,bw=5,kernel="gaussian")
Gaus3<-density(buffalo$X126.4,bw=7,kernel="gaussian")
Gaus4<-density(buffalo$X126.4,bw=15,kernel="gaussian")
plot(Gaus,type='l',main='Annual snowfall in Buffalo, NY - Gaussian kernel',
     xlab='Year',ylab='annual snowfall',
     ylim=c(0,0.02))
lines(Gaus2,col='blue')
lines(Gaus3,col='darkgreen')
lines(Gaus4,col='red')
legend('topright',legend=c('bw=9.131','bw=5','bw=7','bw=15'),
       col=c('black','blue','darkgreen','red'),lty=1)
```
```{r}
#Epanechnikov kernels
Epan<-density(buffalo$X126.4,kernel='epanechnikov')
Epan2<-density(buffalo$X126.4,bw=5,kernel='epanechnikov')
Epan3<-density(buffalo$X126.4,bw=7,kernel='epanechnikov')
Epan4<-density(buffalo$X126.4,bw=15,kernel='epanechnikov')
plot(Epan,type='l',main='Annual snowfall in Buffalo, NY - Epanechnikov kernel',
     xlab='Year',ylab='annual snowfall',
     ylim=c(0,0.02))
lines(Epan2,col='blue')
lines(Epan3,col='darkgreen')
lines(Epan4,col='darkred')
legend('topright',legend=c('bw=9.131','bw=5','bw=7','bw=15'),
       col=c('black','blue','darkgreen','darkred'),lty=1)
```
7. Is the estimate more influenced by the type of kernel or the bandwidth? 

**Answer**
It is more influenced by the bandwidth.
